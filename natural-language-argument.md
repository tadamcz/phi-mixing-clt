...
For parameters $N\ge1$, $a\ge0$, and $s>0$, with $g=aN<1$, introduce the feedback rule
...
**Lemma 1 (Cheap attenuation).** Fix a symmetric square-integrable innovation law and an existing finite cascade with doubling cumulative memories. For every $\tau,\eta,\beta>0$, the cascade has a nonempty finite extension whose output satisfies
...
A continuous odd causal observable $F$ has influence envelope $d=(d_k)_{k\ge0}$ if changing its input at lag $k$ by $u$ changes its value by at most $d_k|u|$. Suppose $d_k\ge0$ and $D=\sum_k d_k<\infty$. Write $S_n(F)=\sum_{t<n}F(\xi_t,\xi_{t-1},\ldots)$. With $v=\|\xi_0\|_2$, the Efron–Stein inequality gives
...
Starting from the current innovation, with envelope $d_0=1$, $d_k=0$ for $k>0$, this proves that every finite cascade has a finite square-root upper bound.
...
|r_s(u)-r_s(u')|
\le w\left(
|\xi_0'-\xi_0|\mathbf1_{\{|\xi_0'-\xi_0|>L\}}
+\frac{L}{s}(|u|+|u'|)
\right).
\tag{10}
$$
...
Given an initial slope $T>0$, choose $L$, and then $A$, so that
...
**Proof.** Each inverse increment has norm at most $\beta_i$ and memory $M_i$, which gives (13). To control the cross terms in (14), set $c_i=\sqrt{M_i}\beta_i$. The matrix multiplying $c_ic_k$ is bounded by $2^{-|i-k|/2}$, whose row sums are at most $3+2\sqrt2<6$. This proves (14), first for finite cascades and then by monotone convergence. $\square$
...
For every $\varepsilon>0$, there is a forgetting time $T$, depending only on the reference process and $\varepsilon$, such that for every $n$,
...
$$
e_\xi(R)=\mathbb E\bigl[\xi^2\mathbf1_{\{|\xi|>R\}}\bigr].
$$
...
$$
S_n=\sum_{t<n}\xi_t+D_n,\qquad |D_n|\le nB.
$$
...
$$
T_n=\sum_{t<n}\xi_t\mathbf1_{\{|\xi_t|>R\}}.
$$
...
The finite-core sums are centered. For every $a>0$, (21), (23), (24), and Chebyshev's inequality imply
